...
count:          ll $s3, 0($a3)          # load counter into $s3
		addi $t4, $s3, 1        # count += 1 
		sc $t4, 0($a3)          # check if job complete 
		beq $t4, $zero, count   # repeat loop if count == 0 
		
		# need code to set i and j and reallocate memory  
...
Loop3:	sll $t0, $s2, 2		# $t0 = 4*k (4 = size of row)
	...
	#		bne $s0, $t1, Loop1	# continue Loop2 if j != 4
		...
		bne $s3, $t3, count  # check to see if new job exists 
